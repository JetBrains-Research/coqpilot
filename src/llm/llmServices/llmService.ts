...
import {
    ...
    GenerationRequest,
    GenerationsLogger,
} from "./utils/generationsLogger/generationsLogger";
...
export abstract class LLMServiceInternal {
    ...
    /**
     * A helper function that wraps `LLMServiceInternal.generateFromChatImpl` call and
     * generation logging with errors handling.
     *
    ...
    readonly generateFromChatWrapped = async <T>(
        ...
        return this.logGenerationAndHandleErrors<T>(
            errorsHandlingMode,
            () => {
                const analyzedChat = buildAndValidateChat();
                return {
    ...
    readonly logGenerationAndHandleErrors = async <T>(
        ...
        generateProofs: (request: GenerationRequest) => Promise<string[]>,
        wrapRawProof: (proof: string) => T
    ): Promise<T[]> => {
        ...
        try {
            ...
            this.generationsLogger.logGenerationSucceeded(request, proofs);
    ...
    /**
     * A helper function to handle unsupported method properly.
     */
    unsupportedMethod(message: string, errorsHandlingMode: ErrorsHandlingMode) {
 ...
 *
 * It's instance is available inside `LLMServiceInternal` and
