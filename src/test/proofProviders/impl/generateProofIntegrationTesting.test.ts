...
import { toProofVersion } from "../proofProvidersSpecificTestUtils/expectGeneratedProof";
import {
    ExpectedRecord,
    expectLogs,
} from "../proofProvidersSpecificTestUtils/expectLogs";
import {
    MockService,
...
} from "../proofProvidersSpecificTestUtils/testFailedGeneration";
 ...
 */
suite("[ProofProvider] Integration testing of `generateProof`", () => {
                        ...
                        metadataHolder,
                        {
                            proofsToGenerateNumber:
                                withFixesMockParams.multiroundProfile
                                    .defaultProofFixChoices,
                            getProof: () => MockService.fixedProofString,
                            getVersionNumber: () => 2,
    ...
    > {
        ...
        for (let w = 0; w < testParams.workersN; w++) {
            const work = async () => {
                const workerMockParams: MockServiceModelParams = {
                    ...basicMockParams,
                    workerId: w,
                };

                let toFixCandidates: MockServiceGeneratedProof[] = [];
