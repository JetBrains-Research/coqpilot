...
import { OpenAiModelParams } from "../../../../proofProviders/impl/modelParams";
import { OpenAiService } from "../../../../proofProviders/impl/openai/openAiService";
import { ConfigurationError } from "../../../../proofProviders/proofProviderErrors";
import { OpenAiUserModelParams } from "../../../../proofProviders/userModelParams";
...
suite("[ProofProvider] Test `OpenAiService`", function () {
    ...
    test("Test `resolveParameters` reads & accepts valid params", async () => {
        ...
        await withProofProvider(new OpenAiService(), async (openAiService) => {
        ...
        test(`Test \`resolveParameters\` resolves tokens with defaults: "${modelName}"${withDefinedTokensLimit} ${tokensLimitSuffix}`, async () => {
            ...
            await withProofProvider(
                ...
                async (openAiService) => {
                    ...
                    expect(
                        resolutionResult.resolved!.maxTokensToGenerate
                    ).toEqual(expectedMaxTokensToGenerate);
                    // check it was resolution with default indeed
                    expect(
                        resolutionResult.resolutionLogs.find(
                            (paramLog) =>
                                paramLog.inputParamName ===
                                "maxTokensToGenerate"
