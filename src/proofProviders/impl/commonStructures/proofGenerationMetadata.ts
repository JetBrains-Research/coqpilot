...
export class ProofGenerationMetadataHolder {
    ...
    updateWithBuiltRequest(builtRequest: ProofProviderRequest) {
        if (this._configuration === undefined) {
            invariantFailed(
    ...
    setSuccessMetadata(requestSucceeded: ProofProviderRequestSucceeded) {
    ...
    setFailureMetadata(requestFailed: ProofProviderRequestFailed) {
        this.throwIfRepeatedSet(this._failure, "failure metadata");
        this._failure = {
            proofProviderError: requestFailed.proofProviderError,
        };
    }
