...
export namespace SchedulersProviderBuilders {
    /**
     * Limits parallelism to the proofProvider for the models with the same key (extracted by `extractKey`).
     */
    ...
    >(
        ...
        >(
            ...
            extractKey,
            (key) =>
                `Models Scheduler: ${proofProviderFullName}, "${key}" / max ${maxParallelRequestsToKey} requests in parallel`,
            enableModelsSchedulingDebugLogs
        );
    }

    /**
     * Limits parallelism to the proofProvider for the models with the same `modelId`.
     * Effectively, parallelism is unlimited.
     */
    export function unlimitedParallelism<
        ResolvedModelParams extends ModelParams,
    >(proofProviderFullName: string, enableModelsSchedulingDebugLogs: boolean) {
        return new UnlimitedSchedulersProvider<ResolvedModelParams>(
            `Models Scheduler: ${proofProviderFullName} Proof Provider, unlimited parallelism`,
            enableModelsSchedulingDebugLogs
        );
    }

    /**
     * Limit parallelism to the proofProvider globally, regardless of the models involved.
     */
    export function limitParallelismGlobally(
        ...
        return new LimitedParallelismSchedulersProvider(
            maxParallelRequests,
            `Models Scheduler: ${proofProviderFullName} Proof Provider, max ${maxParallelRequests} request per time`,
            enableModelsSchedulingDebugLogs
        );
    }
}
