 ...
 * needed to `ProofProvider` to become available again.
 *
...
export class GenerationsLogger {
    ...
    logGenerationSucceeded(request: ProofProviderRequestSucceeded) {
    ...
    logGenerationFailed(request: ProofProviderRequestFailed) {
        let record = new LoggerRecord(
            nowTimestampMillis(),
            request.params.modelId,
            "FAILURE",
            ...
            undefined,
            this.toLoggedError(
                this.extractAndValidateCause(request.proofProviderError)
            )
        );
        if (this.settings.debug) {
            record = new DebugLoggerRecord(
    ...
    private extractAndValidateCause(
        ...
        const cause = proofProviderError.cause;
        if (cause instanceof ProofProviderError) {
            illegalState(
                "received doubled-wrapped error to log, ",
                "cause is instance of `ProofProviderError`: ",
